...
function Update-SetupVersion
{
    ...
    foreach ($o in $input) 
    {
        ...
        $setupVersionPattern = 'ProductVersion</td><td>[0-9]+(\.([0-9]+)){1,3}</td><td/>'
        $setupVersion = 'ProductVersion</td><td>' + $Version + '</td><td/>';

        (Get-Content $o.FullName) | ForEach-Object  { 
           % {$_ -replace $setupVersionPattern, $setupVersion } 
        } | Out-File $o.FullName -encoding UTF8 -force
    }
}
...
function Update-AllAssemblyInfoFiles ( $version )
{
    ...
    foreach ($file in "Setup.isl" ) 
    {
        get-childitem $path -recurse |? {$_.Name -eq $file} | Update-SetupVersion $version ;
    }
}
